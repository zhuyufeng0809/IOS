...
-(UILabel*)titleLabel{
    if(!_titleLabel){
        ...
        if(!_titleLabelFont){
            _titleLabel.font = [UIFont fontWithName:@"HelveticalNeue-Bold" size:16];
        }else{
...
-(UIImageView*)lineImageView{
    if(!_lineImageView){
        ...
        if(!_isThickLine){
            ...
            _lineImageView.image = [[UIImage imageNamed:@"thickhorizonImage"]resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0, 0, 0)];
        }
    }
...
- (void) setViewsLayout{
    if(!_isSexImage){
        [_titleImageView makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.width.equalTo(self.mas_height).multipliedBy(0.64);
            make.height.equalTo(self.mas_height).multipliedBy(0.64);
        }];
    }else{
        [_titleImageView makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.mas_left).offset(1.3);
            make.centerY.equalTo(self.mas_centerY);
            
            make.width.equalTo(self.mas_height).multipliedBy(0.55);
