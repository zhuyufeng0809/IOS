...
-(SettingTitleView*)scanTitleView{
    if(!_scanTitleView){
        ...
        _scanTitleView.titleLabelFont = [UIFont fontWithName:@"Heiti SC" size:20];
    }
...
-(SettingTitleView*)aboutTitleView{
    if(!_aboutTitleView){
        ...
        _aboutTitleView.titleLabelFont = [UIFont fontWithName:@"Heiti SC" size:20];
...
-(void)setViewsLayout{
    ...
    [_logoutButton makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.centerX);
        make.top.equalTo(_aboutButton.bottom).offset(14);
